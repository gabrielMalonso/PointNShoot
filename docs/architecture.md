...
PointNShoot é uma extensão Chrome Manifest V3 sem backend.
...
8. O worker salva o PNG via `chrome.downloads` na subpasta configurada de Downloads, por padrão `PointNShoot-PNG`, aguarda conclusão e confirma o path absoluto com `chrome.downloads.search`.
