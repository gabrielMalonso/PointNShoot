...
describe("diagnostics", () => {
  ...
  it("keeps warning and error diagnostics visible in the console", () => {
    ...
    const warning = makeDiagnostic("background", "warn", "offscreen:close", "Could not close offscreen document.");
    const failure = errorDiagnostic("offscreen", "render:unhandled-error", new Error("render failed"));

    appendDiagnostic([], warning);
    appendDiagnostic([], failure);

    expect(debug).not.toHaveBeenCalled();
    expect(warn).toHaveBeenCalledWith("[PointNShoot]", warning.step, warning.message, {});
    expect(error).toHaveBeenCalledWith("[PointNShoot]", failure.step, failure.message, failure.details);
  });
});
