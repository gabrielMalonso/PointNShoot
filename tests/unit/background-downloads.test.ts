...
import { buildDownloadFilename, getConfiguredDownloadFolder, type DownloadsApi, downloadRenderedPng } from "../../src/background/downloads";
...
function createApi(searchResults: chrome.downloads.DownloadItem[][]): DownloadsApi & {
  ...
  search: ReturnType<typeof vi.fn<(query: chrome.downloads.DownloadQuery) => Promise<chrome.downloads.DownloadItem[]>>>;
} {
...
const baseInput = {
  ...
  requestedFilename: "PointNShoot-PNG/2026-05-25-1900-button-buy-capture1.png",
  imageBytes: 123,
  width: 800,
  height: 600,
};
...
describe("background downloads", () => {
  ...
  it("generates the requested Downloads-relative filename", () => {
    ...
    expect(buildDownloadFilename(request, { now: new Date("2026-05-25T19:07:00") })).toBe(
      "PointNShoot-PNG/2026-05-25-1907-button-data-testid-buy-cta-captur.png",
    );
  });
  ...
  it("allows the Downloads subfolder to be configured safely", async () => {
    await expect(
      getConfiguredDownloadFolder({
        get: vi.fn(async () => ({ pointNShootDownloadFolder: "../Custom Notes//UI Shots" })),
      }),
    ).resolves.toBe("Custom-Notes/UI-Shots");
    await expect(getConfiguredDownloadFolder({ get: vi.fn(async () => ({})) })).resolves.toBe("PointNShoot-PNG");
  });

  it("downloads, waits for completion and returns the absolute filename", async () => {
    const api = createApi([
      [item({ id: 7, state: "in_progress" })],
      [item({ id: 7, state: "complete", filename: "/Users/test/Downloads/PointNShoot-PNG/file.png" })],
    ]);
  ...
  it("uses the real filename returned by Chrome when uniquify changes it", async () => {
    const api = createApi([
      [item({ id: 7, state: "in_progress" })],
      [item({ id: 7, state: "complete", filename: "/Users/test/Downloads/PointNShoot-PNG/file (1).png" })],
    ]);
    ...
    await expect(promise).resolves.toMatchObject({
      filename: "/Users/test/Downloads/PointNShoot-PNG/file (1).png",
      requestedFilename: baseInput.requestedFilename,
    });
  });
