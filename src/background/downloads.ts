...
export const DOWNLOAD_FOLDER = "PointNShoot-PNG";
export const DOWNLOAD_FOLDER_STORAGE_KEY = "pointNShootDownloadFolder";
...
export async function downloadRenderedPng(
  ...
  if (input.imageDataUrl.length > MAX_DOWNLOAD_DATA_URL_BYTES) {
    throw new Error(`download-failed: rendered PNG data URL is too large (${input.imageDataUrl.length} bytes)`);
  }
...
export async function getConfiguredDownloadFolder(storage: Pick<chrome.storage.StorageArea, "get"> | undefined = globalThis.chrome?.storage?.local): Promise<string> {
...
export function waitForDownloadComplete(api: DownloadsApi, downloadId: number, timeoutMs = DOWNLOAD_TIMEOUT_MS): Promise<void> {
  return new Promise((resolve, reject) => {
    let settled = false;
    const timer = setTimeout(() => {
      settle("reject", new Error(`download-failed: download ${downloadId} timed out after ${timeoutMs}ms`));
    }, timeoutMs);
    ...
    const onErased = (erasedId: number): void => {
      if (erasedId === downloadId) {
        settle("reject", new Error(`download-failed: download ${downloadId} was erased before completion`));
      }
    };
    ...
    void api.search({ id: downloadId }).then(
      (items) => {
        ...
        if (item?.state === "interrupted") settle("reject", new Error(`download-failed: download ${downloadId} interrupted`));
      },
      () => {
...
async function getDownloadItem(api: DownloadsApi, downloadId: number): Promise<chrome.downloads.DownloadItem> {
  const items = await api.search({ id: downloadId });
  const item = items[0];

  if (!item) throw new Error(`download-failed: download ${downloadId} was not found after completion`);
  if (!item.filename) throw new Error(`download-failed: download ${downloadId} has no absolute filename`);
