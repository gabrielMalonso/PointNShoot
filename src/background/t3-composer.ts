...
export type T3ComposerDeliveryChrome = {
  tabs: {
    ...
    update(tabId: number, updateProperties: chrome.tabs.UpdateProperties): Promise<T3ComposerDeliveryTab>;
    onUpdated: {
  ...
  scripting: {
    executeScript(details: T3ComposerExecuteScriptDetails): Promise<T3ComposerExecuteScriptResult[]>;
  };
};
...
export async function deliverToT3Composer(
  ...
  const httpDelivery = await deliverPayloadToHttpBridge(fetchApi, payload);
  logT3Delivery(
...
export function buildT3ComposerIntakePayload(input: {
  ...
  return {
    ...
    image: {
      ...
      name: basenameFromPath(input.savedImage.filename) || basenameFromPath(input.savedImage.requestedFilename) || "pointnshoot.png",
      mimeType: "image/png",
...
async function deliverPayloadToHttpBridge(
  ...
  try {
    const response = await fetchApi(DEFAULT_T3_COMPOSER_INTAKE_ENDPOINT, {
      method: "POST",
      headers: {
    ...
    return {
      ...
      message:
        body?.ok === false
          ? (body.message ?? `HTTP ${response.status} ${response.statusText || "response"}: ${reason}`)
...
function sortCandidateTabs(tabs: T3ComposerDeliveryTab[]): T3ComposerDeliveryTab[] {
  return [...tabs]
    ...
    .sort((left, right) => tabScore(left) - tabScore(right));
}
...
function createRequestId(savedImage: SavedImage): string {
  const randomSuffix = crypto.randomUUID ? crypto.randomUUID() : Math.random().toString(16).slice(2);
  return `pointnshoot-${savedImage.downloadId}-${randomSuffix}`;
}
