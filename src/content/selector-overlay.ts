...
const NON_SELECTABLE_TAGS = new Set(["html", "head", "meta", "link", "style", "script", "noscript", "template"]);

export function findPickTarget(clientX: number, clientY: number, host: HTMLElement, doc: Document = document): Element | null {
...
export function isSelectableElement(element: Element, host: HTMLElement): boolean {
  ...
  if (element.closest?.("[data-pointnshoot-root='true']")) return false;
