...
export function appendDiagnostic(entries: DiagnosticLogEntry[], entry: DiagnosticLogEntry): DiagnosticLogEntry[] {
  logDiagnostic(entry);
...
export function formatFallbackText(markdownPrompt: string, diagnostics?: DiagnosticLogEntry[]): string {
  const report = formatDiagnostics(diagnostics);
  if (!report) return markdownPrompt;
  return `${markdownPrompt}\n\n---\nPointNShoot diagnostics:\n${report}`;
}
