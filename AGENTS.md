...
- For code changes, all of `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`, and `pnpm e2e` must pass before considering tasks completed.
- After each code change, run the smallest relevant check first; before considering the task complete, run `pnpm smoke` when a full validation is needed.
- Run `pnpm build` before `pnpm e2e`; the e2e specs depend on `dist`.
- NEVER run `pnpm test:watch` as a completion check. Always use `pnpm test` (Vitest run).

## Project Snapshot

`PointNShoot` is a local Chrome Manifest V3 extension for selecting a page element, adding a note, and copying an annotated PNG for coding agents.
